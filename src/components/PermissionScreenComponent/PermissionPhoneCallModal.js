...
const PermissionPhoneCallModal = ({visible, onPress, onPress1, onPress2}) => {
  const {
    containerStyle,
    imageBoxStyle,
    ...
    <PermissionModalContainer visible={visible} onPress={onPress}>
      <View style={containerStyle}>
        <View style={imageBoxStyle}>
          <Image source={image.phoneCallOrangeIcon} />
        </View>
        <Text style={permissionDescriptionTextStyle}>
          Allow <Text style={appNameTextStyle}>Owe Wisely</Text> to make &
          manage phone calls ?
        </Text>
...
export {PermissionPhoneCallModal};
